...
def preprocess(img_array, preprocess_dict):
	...
	if preprocess_dict["crop"]["crop"]:
		...
		new_img_array = crop(img_array, top=top, bottom=bottom)

	if preprocess_dict["scale_images"]["scale"]:
		scale_factor = preprocess_dict["scale_images"]["scale_factor"]
		new_img_array = scale_images(new_img_array, scale_factor=scale_factor)
	...
	if preprocess_dict["gaussian_blur"]["blur"]:
		...
		new_img_array = gaussian_blur(new_img_array, kernel_size=kernel_size)
	...
	if preprocess_dict["canny_edge_detection"]["detect"]:
		...
		new_img_array = canny(new_img_array, low_threshold=low_threshold, high_threshold=high_threshold)
...
if __name__ == "__main__":
	...
	X_test_processed = preprocess(X_test, preprocess_dict)

	fig = plt.figure(figsize=(30, 10))

	X_test_processed = np.squeeze(X_test_processed)
